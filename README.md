...
these things yet, then run this:
	...
	O$ sudo apt-get install autoconf automake autotools-dev curl libmpc-dev libmpfr-dev libgmp-dev gawk build-essential bison flex texinfo gperf libc6-dev pkg-config bridge-utils uml-utilities zlib1g-dev libglib2.0-dev libtool libsdl1.2-dev
...
The build process for `riscv64-unknown-linux-gnu-gcc` will automatically populate
a sysroot for us. This will be located in `$RISCV/sysroot64`.
...
`riscv-gnu-toolchain` repository, which will be automatically copied to `$RISCV/sysroot64`. 
...
`riscv-tools` repository and run the configure script to generate the 
Makefile.
...
This will place the new cross-compiler in $RISCV/bin and divert the necessary
files to `$RISCV/sysroot64`.
...
Change out of the `riscv-tools/riscv-gnu-toolchain` directory and 
return to $TOP to download the current minor revision of the 3.14 Linux kernel 
series from [The Linux Kernel Archives](http://www.kernel.org), and in one fell
swoop, untar them:
...
Next, we'll pull in the `riscv` architecture specific subtree from the `ucb-bar` GitHub organization:

    $ git init
    $ git remote add origin git@github.com:ucb-bar/riscv-linux.git
    $ git fetch
    $ git checkout -f -t origin/qemu-coredump-demo

Note that we checked out the `qemu-coredump-demo` branch. This contains a defconfig
that works with the current commit of `riscv-qemu` included in `riscv-tools`. If 
you're interested in using Spike, you should instead use the `master` branch of
`riscv-linux`.
